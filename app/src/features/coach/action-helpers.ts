...
export type ActionResult =
  ...
  | { ok: false; redirect: string; error: string };

export class CoachActionRedirect extends Error {
  readonly result: ActionResult;

  constructor(redirectUrl: string, error?: string) {
    super(error ?? redirectUrl);
    this.name = "CoachActionRedirect";
    this.result = error
      ? { ok: false, redirect: redirectUrl, error }
      : { ok: true, redirect: redirectUrl };
  }
}

function actionRedirect(redirectUrl: string, error?: string): never {
  throw new CoachActionRedirect(redirectUrl, error);
}
...
export async function executeSaveQaAnswer(projectId: string, resumeId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) return actionRedirect(buildQaRedirect(projectId, "error", undefined, "resume-not-found"));

  const current = await getProjectResume(project.id, resumeId);
  if (!current) return actionRedirect(buildQaRedirect(project.id, "error", undefined, "resume-not-found"));
  ...
  if (!parsed.success) {
    return actionRedirect(buildQaRedirect(project.id, "error", String(formData.get("questionId") ?? "") || undefined, "qa-invalid"));
  }
  ...
  try {
    ...
    return actionRedirect(buildQaRedirect(project.id, "error", parsed.data.questionId, "qa-persist-failed"));
  }

  return actionRedirect(buildQaRedirect(project.id, "saved", parsed.data.questionId));
}

export async function executePromoteToEvidence(projectId: string, resumeId: string, answerId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) return actionRedirect(buildQaEvidenceRedirect(projectId, "error", undefined, "resume-not-found"));

  const current = await getProjectResume(project.id, resumeId);
  if (!current) return actionRedirect(buildQaEvidenceRedirect(project.id, "error", undefined, "resume-not-found"));
  ...
  try {
    ...
    const code = message.includes("尚未确认")
      ...
      : "qa-answer-not-found";
    return actionRedirect(buildQaEvidenceRedirect(project.id, "error", undefined, code));
  }

  const star = pickStarFromForm(formData);
  if (!star.ok) {
    return actionRedirect(buildQaEvidenceRedirect(project.id, "error", answer.questionId, "star-invalid"));
  }
  ...
  try {
    ...
    return actionRedirect(buildQaEvidenceRedirect(project.id, "error", answer.questionId, "evidence-append-failed"));
  }

  return actionRedirect(buildQaEvidenceRedirect(project.id, "ok", answer.questionId));
}
...
export async function executeConfirmFinding(projectId: string, resumeId: string, reportId: string, findingId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) {
    return actionRedirect(`/projects/${projectId}/coach?confirmStatus=error&confirmCode=resume-not-found&finding=${findingId}`);
  }

  const current = await getProjectResume(project.id, resumeId);
  if (!current) {
    return actionRedirect(`/projects/${project.id}/coach?confirmStatus=error&confirmCode=resume-not-found&finding=${findingId}`);
  }
  ...
  try {
    ...
    return actionRedirect(buildConfirmRedirect(project.id, "error", reportId, findingId, "report-read-failed"));
  }
  if (!report) {
    return actionRedirect(buildConfirmRedirect(project.id, "error", reportId, findingId, "report-not-found"));
  }
  if (report.schemaVersion !== "coach-report-v2") {
    return actionRedirect(buildConfirmRedirect(project.id, "error", reportId, findingId, "report-version-unsupported"));
  }

  const finding = report.findings.find((item) => item.id === findingId);
  if (!finding) {
    return actionRedirect(buildConfirmRedirect(project.id, "error", reportId, findingId, "finding-not-found"));
  }
  if (finding.confirmationStatus === "confirmed") {
    return actionRedirect(buildConfirmRedirect(project.id, "error", reportId, findingId, "finding-already-confirmed"));
  }

  const experienceId = String(formData.get("experienceId") ?? "").trim();
  if (!experienceId) {
    return actionRedirect(buildConfirmRedirect(project.id, "error", reportId, findingId, "experience-not-found"));
  }
  ...
  const star = pickStarFromForm(formData);
  if (!star.ok) {
    return actionRedirect(buildConfirmRedirect(project.id, "error", reportId, findingId, "star-invalid"));
  }
  ...
  try {
    ...
    return actionRedirect(buildConfirmRedirect(project.id, "error", reportId, findingId, "evidence-append-failed"));
  }
  ...
  try {
    ...
    return actionRedirect(buildConfirmRedirect(project.id, "error", reportId, findingId, "report-write-failed"));
  }

  return actionRedirect(buildConfirmRedirect(project.id, "ok", reportId, findingId));
}
...
export async function executeGenerateBulletDraft(projectId: string, resumeId: string, reportId: string, findingId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) {
    return actionRedirect(buildBulletRedirect(projectId, "error", reportId, findingId, "resume-not-found"));
  }

  const current = await getProjectResume(project.id, resumeId);
  if (!current) {
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "resume-not-found"));
  }

  if (!privacyConfirmed(formData)) {
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "privacy-not-confirmed"));
  }
  ...
  try {
    ...
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "report-read-failed"));
  }
  if (!report) {
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "report-not-found"));
  }
  if (report.schemaVersion !== "coach-report-v2") {
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "report-version-unsupported"));
  }

  const finding = report.findings.find((item) => item.id === findingId);
  if (!finding) {
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "finding-not-found"));
  }
  if (finding.confirmationStatus !== "confirmed" || !finding.linkedExperienceId || !finding.linkedEvidenceId) {
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "finding-not-confirmed"));
  }
  if (finding.linkedBulletId) {
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "bullet-already-applied"));
  }

  const experience = current.document.experiences.find((item) => item.id === finding.linkedExperienceId);
  if (!experience) {
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "evidence-not-found"));
  }
  ...
  if (hasPendingDraftForFinding(project.id, resumeId, findingId)) {
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "draft-pending-exists"));
  }
  ...
  try {
    ...
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "missing-model-config"));
  }
  ...
  try {
    ...
    const code = error instanceof CoachBulletProviderError
      ...
      : "provider-failed";
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, code));
  }
  ...
  try {
    ...
    return actionRedirect(buildBulletRedirect(project.id, "error", reportId, findingId, "draft-persist-failed"));
  }

  return actionRedirect(buildBulletRedirect(project.id, "draft", reportId, findingId, undefined, draft.id));
}
...
export async function executeApplyBulletDraft(projectId: string, resumeId: string, draftId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) {
    return actionRedirect(`/projects/${projectId}/coach?confirmStatus=error&confirmCode=resume-not-found`);
  }

  const current = await getProjectResume(project.id, resumeId);
  if (!current) {
    return actionRedirect(`/projects/${project.id}/coach?confirmStatus=error&confirmCode=resume-not-found`);
  }

  const draft = await readBulletDraft(project.id, resumeId, draftId);
  if (!draft) {
    return actionRedirect(`/projects/${project.id}/coach?confirmStatus=error&confirmCode=draft-not-found`);
  }
  if (draft.status !== "pending") {
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "draft-already-applied", draftId));
  }
  ...
  if (!parsedInput.success) {
    const code = parsedInput.error.issues.some((issue) => issue.path[0] === "candidateIndex")
      ...
      : "final-text-invalid";
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, code, draftId));
  }
  if (parsedInput.data.candidateIndex >= draft.candidates.length) {
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "candidate-out-of-range", draftId));
  }

  const experience = current.document.experiences.find((item) => item.id === draft.experienceId);
  if (!experience) {
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "evidence-not-found", draftId));
  }
  ...
  try {
    ...
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "report-read-failed", draftId));
  }
  if (!report || report.schemaVersion !== "coach-report-v2") {
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "report-version-unsupported", draftId));
  }
  const finding = report.findings.find((item) => item.id === draft.findingId);
  if (!finding) {
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "finding-not-found", draftId));
  }
  if (finding.confirmationStatus !== "confirmed") {
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "finding-not-confirmed", draftId));
  }
  if (finding.linkedBulletId) {
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "bullet-already-applied", draftId));
  }
  ...
  try {
    ...
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "evidence-append-failed", draftId));
  }
  ...
  try {
    ...
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "draft-write-failed", draftId));
  }
  ...
  try {
    ...
    return actionRedirect(buildBulletRedirect(project.id, "error", draft.reportId, draft.findingId, "report-write-failed", draftId));
  }

  return actionRedirect(buildBulletRedirect(project.id, "applied", draft.reportId, draft.findingId, undefined, draftId, bulletId));
}

export async function executeGenEvidenceBulletDraft(projectId: string, resumeId: string, experienceId: string, evidenceId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) return actionRedirect(buildEvidenceBulletRedirect(projectId, "error", evidenceId, "resume-not-found"));

  const current = await getProjectResume(project.id, resumeId);
  if (!current) return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, "resume-not-found"));

  if (!privacyConfirmed(formData)) {
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, "privacy-not-confirmed"));
  }

  const experience = current.document.experiences.find((item) => item.id === experienceId);
  if (!experience) return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, "experience-not-found"));
  const evidence = experience.evidence.find((item) => item.id === evidenceId);
  if (!evidence) return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, "evidence-not-found"));
  ...
  if (evidence.actions.length === 0 || evidence.results.length === 0 || !hasConfirmedResult) {
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, "evidence-not-confirmed"));
  }

  const existingDraft = await getActivePendingDraftForEvidence(project.id, resumeId, evidenceId);
  if (existingDraft) {
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, "draft-pending-exists", existingDraft.id));
  }
  ...
  try {
    ...
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, "missing-model-config"));
  }
  if (!defaultConfig?.apiKey) {
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, "missing-model-config"));
  }
  ...
  try {
    ...
    const code = error instanceof CoachBulletProviderError
      ? (error.code === "timeout" ? "provider-timeout" : error.code)
      : "provider-failed";
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, code));
  }
  ...
  try {
    ...
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", evidenceId, "draft-persist-failed"));
  }

  return actionRedirect(buildEvidenceBulletRedirect(project.id, "draft", evidenceId, undefined, draft.id));
}

export async function executeApplyEvidenceBulletDraft(projectId: string, resumeId: string, draftId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) return actionRedirect(`/projects/${projectId}/coach?evidenceBulletStatus=error&evidenceBulletCode=resume-not-found`);

  const current = await getProjectResume(project.id, resumeId);
  if (!current) return actionRedirect(`/projects/${project.id}/coach?evidenceBulletStatus=error&evidenceBulletCode=resume-not-found`);

  const draft = await readBulletDraft(project.id, resumeId, draftId);
  if (!draft) return actionRedirect(`/projects/${project.id}/coach?evidenceBulletStatus=error&evidenceBulletCode=draft-not-found`);
  if (draft.source !== "experience_evidence") {
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", draft.evidenceId, "draft-source-invalid", draftId));
  }
  if (draft.status !== "pending") {
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", draft.evidenceId, "draft-already-applied", draftId));
  }
  ...
  if (!parsedInput.success) {
    const code = parsedInput.error.issues.some((issue) => issue.path[0] === "candidateIndex")
      ? "candidate-out-of-range"
      : "final-text-invalid";
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", draft.evidenceId, code, draftId));
  }
  if (parsedInput.data.candidateIndex >= draft.candidates.length) {
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", draft.evidenceId, "candidate-out-of-range", draftId));
  }

  const experience = current.document.experiences.find((item) => item.id === draft.experienceId);
  if (!experience) return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", draft.evidenceId, "experience-not-found", draftId));
  ...
  if (sourceEvidenceIds.some((id) => !experience.evidence.some((item) => item.id === id))) {
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", draft.evidenceId, "evidence-not-found", draftId));
  }
  ...
  try {
    ...
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", draft.evidenceId, "bullet-append-failed", draftId));
  }
  ...
  try {
    ...
    await removeExperienceBullet({ projectId: project.id, resumeId, experienceId: draft.experienceId, bulletId });
    return actionRedirect(buildEvidenceBulletRedirect(project.id, "error", draft.evidenceId, "draft-write-failed", draftId));
  }

  return actionRedirect(buildEvidenceBulletRedirect(project.id, "applied", draft.evidenceId, undefined, draftId, bulletId));
}
...
export async function executeGeneratePolish(projectId: string, resumeId: string, experienceId: string, bulletId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) return actionRedirect(buildPolishRedirect(projectId, "error", "missing-project"));
  if (!privacyConfirmed(formData)) return actionRedirect(buildPolishRedirect(project.id, "error", "privacy-not-confirmed"));

  const current = await getProjectResume(project.id, resumeId);
  if (!current) return actionRedirect(buildPolishRedirect(project.id, "error", "missing-resume"));
  const experience = current.document.experiences.find((item) => item.id === experienceId);
  const sourceBullet = experience?.bullets.find((item) => item.id === bulletId && item.status === "confirmed");
  if (!experience || !sourceBullet) return actionRedirect(buildPolishRedirect(project.id, "error", "source-bullet-not-found"));

  let defaultConfig;
  try {
    defaultConfig = await getDefaultModelConfig();
  } catch {
    return actionRedirect(buildPolishRedirect(project.id, "error", "missing-model-config"));
  }
  if (!defaultConfig?.apiKey) return actionRedirect(buildPolishRedirect(project.id, "error", "missing-model-config"));
  ...
  try {
    ...
    return actionRedirect(buildPolishRedirect(project.id, "error", "provider-failed"));
  }
  ...
  try {
    ...
    return actionRedirect(buildPolishRedirect(project.id, "generated", undefined, run.id));
  } catch {
    return actionRedirect(buildPolishRedirect(project.id, "error", "persist-failed"));
  }
}

export async function executeApplyPolish(projectId: string, resumeId: string, runId: string, candidateId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) return actionRedirect(buildPolishRedirect(projectId, "error", "missing-project", runId));
  const run = await readPolishRun(project.id, resumeId, runId);
  if (!run) return actionRedirect(buildPolishRedirect(project.id, "error", "run-not-found", runId));
  const candidate = run.candidates.find((item) => item.id === candidateId && item.status === "ready");
  if (!candidate) return actionRedirect(buildPolishRedirect(project.id, "error", "candidate-not-found", runId));
  const finalText = String(formData.get("finalText") ?? candidate.text).trim();
  if (!finalText) return actionRedirect(buildPolishRedirect(project.id, "error", "final-text-invalid", runId));
  ...
  try {
    ...
    return actionRedirect(buildPolishRedirect(project.id, "error", "apply-failed", runId));
  }
  return actionRedirect(buildPolishRedirect(project.id, "applied", undefined, runId));
}

export async function executeDiscardPolish(projectId: string, resumeId: string, runId: string, candidateId: string) {
  const project = getProject(projectId);
  if (!project) return actionRedirect(buildPolishRedirect(projectId, "error", "missing-project", runId));
  const run = await readPolishRun(project.id, resumeId, runId);
  if (!run) return actionRedirect(buildPolishRedirect(project.id, "error", "run-not-found", runId));
  if (!run.candidates.some((item) => item.id === candidateId)) return actionRedirect(buildPolishRedirect(project.id, "error", "candidate-not-found", runId));
  await writePolishRun({
    ...run,
    candidates: run.candidates.map((item) => item.id === candidateId ? { ...item, status: "discarded" } : item),
  });
  return actionRedirect(buildPolishRedirect(project.id, "discarded", undefined, runId));
}

export async function executeSearchEvaluation(projectId: string, formData: FormData) {
  const project = getProject(projectId);
...
export async function executeGrillEnhancement(projectId: string, resumeId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) return actionRedirect(buildGrillEnhancementRedirect(projectId, "error", "missing-project"));
  if (!privacyConfirmed(formData)) return actionRedirect(buildGrillEnhancementRedirect(project.id, "error", "privacy-not-confirmed"));

  const current = await getProjectResume(project.id, resumeId);
  if (!current) return actionRedirect(buildGrillEnhancementRedirect(project.id, "error", "missing-resume"));
  ...
  if (!session.base.activeTurn) return actionRedirect(buildGrillEnhancementRedirect(project.id, "error", "missing-active-turn"));

  let defaultConfig;
  try {
    defaultConfig = await getDefaultModelConfig();
  } catch {
    return actionRedirect(buildGrillEnhancementRedirect(project.id, "error", "missing-model-config"));
  }
  if (!defaultConfig?.apiKey) return actionRedirect(buildGrillEnhancementRedirect(project.id, "error", "missing-model-config"));
  ...
  if (!enhancement) return actionRedirect(buildGrillEnhancementRedirect(project.id, "error", "unavailable"));
  ...
  try {
    ...
    return actionRedirect(buildGrillEnhancementRedirect(project.id, "error", "persist-failed"));
  }
  return actionRedirect(buildGrillEnhancementRedirect(project.id, "generated"));
}

export async function executeCoachResearch(projectId: string, formData: FormData) {
  const project = getProject(projectId);
  if (!project) {
    return actionRedirect(`/projects/${projectId}/coach?researchError=missing-project`);
  }

  const resumes = listResumes(project.id);
  const master = firstResume(resumes, "master");
  if (!master) {
    return actionRedirect(`/projects/${project.id}/coach?researchError=missing-resume`);
  }

  if (!privacyConfirmed(formData)) {
    return actionRedirect(`/projects/${project.id}/coach?researchError=privacy-not-confirmed`);
  }
  ...
  if (selectedIds.length === 0) {
    return actionRedirect(`/projects/${project.id}/coach?researchError=no-selection`);
  }
  ...
  try {
    ...
    return actionRedirect(`/projects/${project.id}/coach?researchError=resume-read-failed`);
  }
  ...
  if (selectedIds.some((id) => !validIds.has(id))) {
    return actionRedirect(`/projects/${project.id}/coach?researchError=invalid-selection`);
  }
  ...
  try {
    ...
    return actionRedirect(buildProviderErrorRedirect(project.id, "missing-model-config", selectedIds));
  }
  if (!defaultConfig?.apiKey) {
    return actionRedirect(buildProviderErrorRedirect(project.id, "missing-model-config", selectedIds));
  }
  ...
  try {
    ...
    return actionRedirect(buildProviderErrorRedirect(project.id, safeCode === "timeout" ? "provider-timeout" : safeCode, selectedIds));
  }
  ...
  try {
    ...
    return actionRedirect(buildProviderErrorRedirect(project.id, "report-persist-failed", selectedIds));
  }
  return actionRedirect(`/projects/${project.id}/coach?researchStatus=provider&report=${report.id}`);
}
