...
export async function getReachableSteps(profileId: string): Promise<StepKey[]> {
  ...
  if (hasProfile) reachable.push("profile");

  const hasBullet = profile.experiences.some((e) => e.bullets.length > 0);
  if (hasBullet) reachable.push("evaluate");
